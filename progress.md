...
## 2026-09-01/02 — Правки заказчика, рассылка участникам и локализация на 3 языка

#done #i18n

Один длинный заход: сначала список правок от заказчика, потом трёхъязычность.
Архитектура локализации вынесена отдельно — [[i18n]].

### 1. Правки по списку заказчика (коммит `2b53627`)

**Автоскрытие турниров после дедлайна.** Каталог отбирает турниры по
`registrationDeadline >= сегодня`, а не по дате окончания: витрина показывает
только то, куда ещё можно подать заявку. Организатору всё остаётся — «Мои
турниры», список участников и страница турнира строятся другими запросами.
Граница та же, что у кнопки регистрации: весь день дедлайна турнир ещё виден.

**Комитеты MUN с описанием.** Был `<select>` с одними заголовками, и участник
выбирал вслепую — чтобы прочитать повестку, надо было уходить со страницы
регистрации и терять заполненную форму. Теперь карточки с названием И
описанием, выбор настоящими радиокнопками (работают стрелки, скринридер
объявляет «2 из 6», браузер сам не даёт отправить без выбора).

**🐛 Ссылки на соцсети открывались как путь внутри сайта.** Организатор вводит
`@mydebate`, а это подставлялось в `href` как есть — браузер считал значение
относительным путём и открывал 404 вместо профиля. `lib/social.ts`
приводит ник / домен / полный URL к рабочему адресу **на выводе**, поэтому
чинятся и уже сохранённые турниры. Заодно добавлено поле **TikTok** (иконка в
проекте была, поля не было — видимо, поэтому ссылку клали в Telegram);
миграция `20260901120000_tournament_tiktok`.

**Кликабельная карточка турнира.** Нажимается вся карточка, а не только
«Подробнее». Приём stretched link: ссылка на заголовке, её `::after` растянут
на карточку. Сердечко избранного поднято над слоем и продолжает работать;
дублирующая кнопка убрана из клавиатурной навигации, чтобы на карточку не
было двух одинаковых табстопов.

**Рассылка участникам** (`lib/actions/announcements.ts`). Организатор пишет
одно сообщение — оно уходит выбранной группе в личный кабинет и на почту.
Пять шаблонов (чат турнира / объявление / расписание / напоминание об оплате /
благодарность), выбор аудитории по статусу заявки, лимит 5 рассылок в час,
письма уходят фоном через `after()`. Ссылки в письме кликабельны, переносы
строк сохраняются — ради ссылки на WhatsApp-группу это и просили.

### 2. Демо-данные (коммит `4613c3a`)
Даты в `prisma/seed.ts` были рассчитаны на «сегодня = 2026-07-05» и
устарели: после нового правила каталога пересев оставлял почти пустой список.
Переведены на смещения от текущего дня. Год в названии прошлого MUN теперь
берётся из вычисленной даты.

### 3. Локализация: русский / қазақша / english

Пять срезов, каждый — словарь на три языка + правка файлов + тесты + выкатка:

1. `35cc59c` каркас next-intl, шапка, подвал, главная, каталог, карточка
2. `366175f` фильтры каталога, страница турнира, выбор даты
3. `7be12b1` вход, регистрация, восстановление пароля
4. `5e5a175` сообщения валидации (63 штуки)
5. `3a195ca` регистрация на турнир
6. `283bd9e` личный кабинет

Все маршруты переехали под `app/[locale]/`. Русский остался на прежних
адресах, казахский и английский получили префикс — подробности и обоснование
в [[i18n]].

### 🐛 Прод лёг на 6 минут — и это моя ошибка

Выкатил каркас i18n, главная ушла в бесконечный редирект `/` → `/`. Страницы
с префиксом при этом работали. Откатил (`4d7ffac`), нашёл причину, починил
(`7089132`), вернул (`f950bfc`).

Причина: внутри обёртки `auth()` NextAuth подменяет origin запроса на
`AUTH_URL`, и next-intl строит из него адрес внутреннего rewrite. Next видит
rewrite на чужой origin и превращает его в редирект. Локально не
воспроизводится — ломается только за обратным прокси.

**Что стоит запомнить:** я видел этот симптом ЗАРАНЕЕ. При проверке на порту
3100 с `AUTH_URL` на 3000 главная отдавала 500, и в логе прямо было написано
`Failed to proxy http://localhost:3000/ru`. Я списал это на несовпадение
порта, перезапустил на 3000, получил зелёный результат и пошёл дальше. Это был
тот же баг. Теперь расхождение origin проверяется явно перед каждой выкаткой
и закреплено тестом — см. [[i18n]].

### 🐛 Кончилось место на сервере
Деплой упал на `git fetch` с `No space left on device`; сайт работал на старом
контейнере. Две причины, обе в `deploy/update.sh` (коммит `a23abd1`):
- `docker image prune -f` удаляет только образы **без тега**, а каждая
  выкатка скачивает образ с тегом коммита — такие не чистились никогда;
- чистка стояла **в конце**, после `git fetch` и `docker compose pull`,
  которым место как раз и нужно: до неё просто не доходило.

Чистка перенесена в начало и сделана адресной. Скрипт приходит на сервер через
stdin, поэтому исправленная версия отработала сразу и вылечила сервер сама.
Теперь в каждом логе деплоя печатается свободное место.

### 🐛 Вёрстка, которая ломается только на переводе
В кабинете заголовок блока и ссылка «смотреть все» стояли в одной строке через
`justify-between` без переноса. На русском помещались, на казахском —
«Таңдаулы» налезало на «Барлығын қарау». Добавлен `flex-wrap` во всех трёх
таких блоках, а не только в том, где заметили.

### Проверка
- 76 юнит-тестов (было 38), 23 e2e (было 7), build / lint / tsc — зелёные на
  каждом срезе.
- Новые тесты: `tests/social.test.ts`, `tests/announcements.test.ts`,
  `tests/i18n.test.ts`, `tests/proxy-i18n.test.ts`, `e2e/i18n.spec.ts`,
  `e2e/tournament-ux.spec.ts`.
- Каждый срез проверен в браузере на всех трёх языках и выкачен на прод.
- Всего за заход: 120 файлов, +5657 / −1027.

### Дальше
См. [[tasks]] — осталась админка, мастер создания турнира, панель рассылки,
письма и статические страницы. И вычитка казахского носителем перед тем, как
открывать `/kk` и `/en` поисковику.

---

## 2026-09-02 — Локализация доведена до конца

#done #i18n

Продолжение вчерашнего (см. запись за 01–02 сентября). Четыре среза перевода
плюс возня с деплоем. Архитектура — в [[i18n]].

### Переведено
- `caad177` «О нас», «Контакты», «Помощь» — этим закрылась вся индексируемая
  часть сайта. FAQ перестроен: `faq-data.ts` хранит только порядок и
  идентификаторы вопросов, тексты в словаре. Важная деталь: **поиск по FAQ
  теперь фильтрует переведённые строки**, а не русский оригинал — иначе на
  казахской странице он не находил бы то, что на ней написано.
- `4048ab4` мастер создания и форма редактирования турнира.
- `64aea40` список участников, карточка заявки, панель рассылки с пятью
  шаблонами (тексты писем целиком уехали в словарь).
- `a484f4a` админ-панель: обзор, модерация, пользователи, организаторы,
  обращения.
- `813c62c` остатки: страницы ошибок, редактор разделов, загрузка обложки.

**Интерфейс переведён полностью.** По-русски осталось ровно то, что и должно:
`console.error` (серверные логи), значения языков турнира и «Казахстан» в
schema.org (данные, а не подписи), юридические документы (по решению
заказчика) и контент из базы, который пишет организатор.

Словарь: 800 строк в 28 namespace на каждый из трёх языков.

### 🐛 Казахские даты в браузере — «2026 M12 30»
Chrome заявляет поддержку `kk-KZ` (`supportedLocalesOf` её возвращает), но
данных по казахскому в его ICU нет: вместо названия месяца печатается «M12».
В Node ICU полный, поэтому серверные компоненты показывали дату правильно, а
клиентские — нет; у компонента, отрисованного и там и там, разметка при
гидрации не совпала бы.

```
Node:    2026 ж. 30 желтоқсан
Chrome:  2026 M12 30
```

Казахские месяцы и дни недели заданы своей таблицей в `lib/format.ts`,
русский и английский остались на `Intl`. Закреплено `tests/format.test.ts`.

### 🐛 Шапка ломалась на казахском
У вошедшего администратора подписи не помещались даже на 1440px: пункты меню
переносились по словам и налезали на логотип. Кнопкам добавлен
`whitespace-nowrap` (подпись кнопки не должна ломаться пополам ни на каком
языке), логотипу — `shrink-0`, порог десктопной шапки поднят с 1024 до 1280.

Это уже второй дефект вёрстки, который вылез только на переводе — первым был
заголовок, налезающий на ссылку в кабинете. Общее у них одно: на русском текст
помещался, на казахском нет.

### 🐛 Деплой встал: сервер требовал логин к публичному репозиторию
`fatal: could not read Username for 'https://github.com'` — три выкатки
подряд. Репозиторий публичный, авторизация для `fetch` не нужна вовсе.

Я потратил **две выкатки на догадки**: сначала вычистил возможный «user@» из
адреса remote (`6816c9a`), потом отключил хранилище учётных данных
(`2fb7283`). Ни то ни другое не помогло. Тогда, по решению заказчика, сделал
неудачный `fetch` некритичным (`1552930`): приложение приезжает образом из
GHCR, а репозиторий на сервере нужен только ради `docker-compose.yml`,
`Caddyfile` и `deploy/*`.

**И на следующей же выкатке fetch сработал сам.** Похоже, причина была во
временных проблемах GitHub: в те же минуты у меня дважды отваливался их API с
`TLS handshake timeout`. То есть обе мои правки чинили не ту проблему.

Вывод на будущее: при непонятной ошибке сети сначала проверять, не лежит ли
сервис, и только потом лезть в конфиг. Правки при этом оставлены — они делают
запрос строго анонимным, что для публичного репозитория правильно, — а
некритичный `fetch` печатает расхождение между коммитом конфига на сервере и
развёрнутым образом, чтобы рассинхрон не случился молча.

### Проверка
- 81 юнит-тест (было 76), 23 e2e, build / lint / tsc — зелёные на каждом срезе.
- Каждый срез проверен в браузере на казахском и выкачен на прод.
- 🐛 Попутно починен флейки-тест: поиск MUN-турнира с комитетами проверял
  заголовок сразу после перехода и иногда попадал на скелетон. Тест не падал —
  он уходил в `skip`, то есть проверка молча переставала выполняться, а прогон
  выглядел зелёным.
- За заход: 55 файлов, +2120 / −694.

### Дальше
Осталось из i18n: шаблоны писем и статусные сообщения экшенов. И вычитка
казахского носителем — до неё `/kk` и `/en` остаются закрытыми от индексации.
Подробности в [[tasks]].
